...
- **GitHub API Integration**: Fetches repository records from `https://api.github.com/users/Tamizh-code/repos`. Filters out forks and excluded meta-repositories, transforming raw API responses into project objects (`title`, `body`, `tags`, `htmlUrl`). Falls back gracefully to `localFallbackProjects` if API limits are met.
